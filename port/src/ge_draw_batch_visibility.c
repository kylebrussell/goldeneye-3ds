    ...
    int matrix_validated)
{
    ...
    for (column = 0U; column < 4U; ++column) {
        ...
        for (axis = 0U; axis < 3U; ++axis) {
            ...
            if (!matrix_validated && !isfinite(coefficient))
                return GE_DRAW_BATCH_BOUNDS_UNCERTAIN;
        ...
        if (!matrix_validated && !isfinite(world_to_clip[3][column]))
            return GE_DRAW_BATCH_BOUNDS_UNCERTAIN;
...
GeDrawBatchBoundsVisibility ge_draw_batch_world_bounds_classify(
    const GeDrawBatchWorldBounds *bounds, const float world_to_clip[4][4])
{
    return ge_draw_batch_world_bounds_classify_impl(bounds, world_to_clip, 0);
}
