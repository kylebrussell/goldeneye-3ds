...
test_dir="${repo_dir}/build/host-tests/bunker2-monitor-owners"
mkdir -p "${test_dir}"
...
cc -std=gnu11 -Wall -Wextra -Werror \
    ...
    -iquote "${repo_dir}" \
    "${repo_dir}/port/tests/test_ge_original_bunker2_monitor_owners.c" \
    ...
    -lm "${dead_strip[@]}" \
    -o "${test_dir}/test_ge_original_bunker2_monitor_owners"
"${test_dir}/test_ge_original_bunker2_monitor_owners" "${asset_pack}"
