...
build_dir="${repo_dir}/build/host-tests/gunbarrel-bond-live"
...
flags=(
    ...
    -DGE_PORT_MS_INHERITS -DVERSION_US -DBUGFIX_R0
    -DGE_PORT_GUNBARREL_HOLE_SLICE -fms-extensions
...
sources=(
    "${build_dir}/model.c"
    "${repo_dir}/port/src/ge_original_gunbarrel_bond.c"
    ...
    "${repo_dir}/port/tests/test_ge_original_gunbarrel_bond_live.c"
)
...
cc "${objects[@]}" -lm -fsanitize=address,undefined "${dead_strip[@]}" \
    -o "${build_dir}/test_ge_original_gunbarrel_bond_live"
ASAN_OPTIONS=detect_leaks=0 UBSAN_OPTIONS=halt_on_error=1 \
    "${build_dir}/test_ge_original_gunbarrel_bond_live" "${pack}"
